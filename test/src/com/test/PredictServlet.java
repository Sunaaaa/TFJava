...
public class PredictServlet extends HttpServlet {
	...
	private SavedModelBundle sModel = SavedModelBundle.load("C:/Python_ML/TFJavaAPI/SaveModel", "serve");
    /**
	...
	private int findMaxNumber(float[] value, int valueLenght, int maxIdx) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			float[][] y = sess.runner().feed("x",x).fetch("h").run().get(0).copyTo(new float[1][10]);
			
			float[] value = y[0];
			int valueLength = 10;
			int maxInx = 0;
			
			result = findMaxNumber(value, valueLength, maxInx);
